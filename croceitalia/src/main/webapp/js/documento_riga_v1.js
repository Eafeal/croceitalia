...
//Contr. Obbligatorietà
		...
		if(rigaForm.km_percorso_s.value == ""){
			alert("I kilometri del percorso e' obbligatorio!");
			rigaForm.km_percorso_s.focus();
			return false;
		}
		if(rigaForm.num_sedute.value == ""){
			alert("Il numero delle sedute e' obbligatorio!");
...
// Contr. Validità
		if (cf_contieneSoloCaratteriValidi(rigaForm.km_percorso_s, ".,1234567890") == 0) return false;
		...
		if (!confirm('Confermi inserimento riga?')){
			return false;
		}
...
var kmpercorso = eval(pulisciNumero(rigaForm.km_percorso_s.value));

var km_totali = numsedute * kmpercorso;
km_totali = (Math.round(km_totali * 100) /100)+"";
rigaForm.km_totali_s.value = km_totali.replace(".",",");
