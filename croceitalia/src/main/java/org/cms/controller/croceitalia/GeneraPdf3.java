...
public class GeneraPdf3 {
	...
	// public GeneraPdf2(String src, String destIn, String destOut,String
	// intestazione1,
	// String intestazione2,String intestazione3,String intestazione4,String
	// intestazione5,String intestazione6,String intestazione7,String
	// intestazione8,String intestazione9) {
	//
	// super();
	// _src = src;
	// _destIn = destIn;
	...
	public GeneraPdf3(String nomeFile, Documento_Testata testata, List<Documento_Righe> righe) {
	...
	private void scriviCliente(PdfContentByte cb, PdfLayer nested, Documento_Testata testata) {
		...
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT,
				new Phrase(testata.getCliente().getRagione_sociale(), smallFont), 270, 655, 0);

		cb.endLayer();
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT,
				new Phrase("Viale/Via " + testata.getCliente().getVia(), smallFont), 270, 642, 0);

		cb.endLayer();
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT,
				new Phrase(testata.getCliente().getCap() + " " + testata.getCliente().getComune(), smallFont), 270, 629,
				0);
		...
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT,
				new Phrase("P.I.: " + testata.getCliente().getPartitaIva(), smallFont), 270, 603, 0);
	...
	private void scriviDate(Documento_Testata testata, PdfContentByte cb, String numero, PdfLayer nested) {
		...
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase(testata.getMese_documento(), smallFont), 488, 750,
				0);
	...
	private void scriviSomme(Integer importo, PdfContentByte cb, PdfLayer nested, Documento_Testata testata) {
		...
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase(testata.getImponibile().toString(), smallFont),
				539, 150, 0);

		cb.endLayer();

		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase(testata.getIva().toString(), smallFont), 539, 126,
				0);
		...
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase(testata.getTotale().toString(), smallFont), 539,
				62, 0);
	...
	private void scriviBonifico(PdfContentByte cb, PdfLayer nested, Documento_Testata testata) {
		...
		cb.endLayer();
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase("COORDINATE BANCARIE:", piusmallFont), 27, 119,
				0);
		...
		ColumnText
				.showTextAligned(
						cb, Element.ALIGN_LEFT, new Phrase(testata.getBanca().getVia() + " "
								+ testata.getBanca().getCap() + " " + testata.getBanca().getComune(), piusmallFont),
						27, 94, 0);

		cb.endLayer();
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase("IBAN: " + testata.getBanca().getIban(), bigFont),
				27, 69, 0);
	...
	private void cellaPercorsoRitorno(List<Documento_Righe> row, PdfContentByte cb, PdfLayer nested) {
		...
		// //////////// percorso ritorno/////////////
		/*
		 * cb.beginLayer(nested); ColumnText.showTextAligned(cb,
		 * Element.ALIGN_LEFT, new Phrase(row.get(_indiceRiga).getP_arrivo() +
	...
	private void cellaAndata(List<Documento_Righe> row, PdfContentByte cb, PdfLayer nested) {
		...
		/*
		 * cb.beginLayer(nested); ColumnText.showTextAligned(cb,
		 * Element.ALIGN_LEFT, new Phrase(row.get(_indiceRiga).getP_partenza() +
