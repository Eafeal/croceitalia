...
public class GeneraPdf2 {
	 ...
	 */
	public void manipulatePdf(int numPagina, List<Documento_Righe> row, List<Documento_Testata> testata, Integer importo,
	...
	private void scriviRighe(int numPagina, List<Documento_Righe> row, PdfContentByte cb, PdfLayer nested) {
		...
		for (int i = 1; i <= numeroRigheDaStampare; i++) {
			...
			cb.beginLayer(nested);
			ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase(row.get(i).getFk_id_paziente().toString(), piusmallFont),
					158, b - a, 0);
			cb.beginLayer(nested);
	...
	private void cellaNome(List<Documento_Righe> row, PdfContentByte cb, PdfLayer nested) {
		...
		cb.beginLayer(nested);
		ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, new Phrase(row.get(0).getFk_id_paziente().toString(), piusmallFont),
				158, 544, 0);
		cb.beginLayer(nested);
	}
